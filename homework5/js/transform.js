...
var MVPmat = function ( dispParams ) {
	...
	function computeViewTransformFromQuatertion( state, halfIpdShift ) {
        ...
        // rotations to adjust axis and orientation from world
        var rotate = new THREE.Quaternion( 0, 1, 0, 0 );
        var quaternion = state.imuQuaternion.clone();
        quaternion.premultiply(rotate).multiply( rotate );
        quaternion.inverse();

        var rotationMat = new THREE.Matrix4().makeRotationFromQuaternion( quaternion );
	...
	function computeViewTransformFromQuatertionWithHeadNeckmodel(
        ...
        viewMat.premultiply(T_pos).premultiply(rotationMat).premultiply(T_neg);
